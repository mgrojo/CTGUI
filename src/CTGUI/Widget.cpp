...
unsigned int tguiWidget_signalFileDialogPathsConnect(tguiWidget* widget, const char* signalName, void (*function)(size_t, tguiUtf32*))
{
    return ctgui::connectSignal<tgui::SignalFileDialogPaths>(widget, signalName, [function](const std::vector<tgui::Filesystem::Path>& value) {
        ...
        for (auto& str : cppStrings)
            cStrings.emplace_back(ctgui::fromCppStr(str));
...
unsigned int tguiWidget_signalItemHierarchyConnect(tguiWidget* widget, const char* signalName, void (*function)(size_t, tguiUtf32*))
{
...
tguiWidget* tguiWidget_getToolTip(const tguiWidget* widget)
{
    ...
    if (widget)
        return ctgui::addWidgetRef(toolTip);
    else
        return nullptr;
}
...
{
    auto returnedWidget = widget->This->getNavigationUp();
    if (returnedWidget)
        return ctgui::addWidgetRef(returnedWidget);
    else
        return nullptr;
}
...
{
    auto returnedWidget = widget->This->getNavigationDown();
    if (returnedWidget)
        return ctgui::addWidgetRef(returnedWidget);
    else
        return nullptr;
...
{
    auto returnedWidget = widget->This->getNavigationLeft();
    if (returnedWidget)
        return ctgui::addWidgetRef(returnedWidget);
    else
        return nullptr;
...
{
    auto returnedWidget = widget->This->getNavigationRight();
    if (returnedWidget)
        return ctgui::addWidgetRef(returnedWidget);
    else
        return nullptr;
