...
tguiWidget* tguiContainer_getWidgetAtPos(tguiWidget* container, float x, float y, tguiBool recursive)
{
    const auto leafWidgetAtPos = DOWNCAST(container->This)->getWidgetAtPos({x, y}, recursive != 0);
    if (leafWidgetAtPos)
...
tguiBool tguiContainer_focusNextWidget(tguiWidget* container)
{
    return DOWNCAST(container->This)->focusNextWidget();
}

tguiBool tguiContainer_focusPreviousWidget(tguiWidget* container)
{
