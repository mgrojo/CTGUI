...
tguiBool tguiFloatRect_contains(const tguiFloatRect* rect, float x, float y)
{
    return tgui::FloatRect{rect->left, rect->top, rect->width, rect->height}.contains({x, y});
}

tguiBool tguiIntRect_contains(const tguiIntRect* rect, int x, int y)
{
    return tgui::IntRect{rect->left, rect->top, rect->width, rect->height}.contains({x, y});
}

tguiBool tguiUIntRect_contains(const tguiUIntRect* rect, unsigned int x, unsigned int y)
{
    return tgui::UIntRect{rect->left, rect->top, rect->width, rect->height}.contains({x, y});
}
